...
namespace WindowsFormsApp1
{
    public partial class schedule : Form
    {
        ...
        {
            string strSql = "SELECT * FROM snlrequest";
            OleDbConnection connection = new OleDbConnection(cnnoledb.ConnectionString);
            OleDbCommand cmd = new OleDbCommand(strSql, connection);
            cmd.CommandType = CommandType.Text;
            OleDbDataAdapter da = new OleDbDataAdapter(cmd);
            DataTable timetable = new DataTable();
            da.Fill(timetable);
            requestdataGridView.DataSource = timetable;
        }

        private void bookmodifybutton_Click(object sender, EventArgs e)
        {
            cmddelete.CommandText = "Delete From snlrequest Where ID ='" + idtextBox.Text + "';";
